...
class VigorDatasetTrain(Dataset):
    ...
    def __init__(self,
        ...
        for city in self.cities:
            df_tmp = pd.read_csv(f'/kaggle/working/Sample4Geo/sample4geo/data/VIGOR{city}/satellite_list.txt', header=None, delim_whitespace=True)
            df_tmp = df_tmp.rename(columns={0: "sat"})
            df_tmp["path"] = df_tmp.apply(lambda x: f'{data_folder}/{city}/satellite/satellite/{x.sat}', axis=1)
        ...
        for city in self.cities:
            ...
            if same_area:
                df_tmp = pd.read_csv(f'/kaggle/working/Sample4Geo/sample4geo/data/VIGOR/{city}/same_area_balanced_train.txt', header=None, delim_whitespace=True)
            else:
                df_tmp = pd.read_csv(f'/kaggle/working/Sample4Geo/sample4geo/data/VIGOR/{city}/pano_label_balanced.txt', header=None, delim_whitespace=True)
            ...
            df_tmp["path_ground"] = df_tmp.apply(lambda x: f'{data_folder}/{city}/panorama/panorama/{x.ground}', axis=1)
            df_tmp["path_sat"] = df_tmp.apply(lambda x: f'{data_folder}/{city}/satellite/satellite/{x.sat}', axis=1)
...
class VigorDatasetEval(Dataset):
    ...
    def __init__(self,
        ...
        for city in self.cities:
            ...
            if same_area:
                df_tmp = pd.read_csv(f'/kaggle/working/Sample4Geo/sample4geo/data/VIGOR/{city}/same_area_balanced_{split}.txt', header=None, delim_whitespace=True)
            else:
                df_tmp = pd.read_csv(f'/kaggle/working/Sample4Geo/sample4geo/data/VIGOR/{city}/pano_label_balanced.txt', header=None, delim_whitespace=True)
            ...
            df_tmp["path_sat_np1"] = df_tmp.apply(lambda x: f'{data_folder}/{city}/satellite/satellite/{x.sat_np1}', axis=1)
            df_tmp["path_sat_np2"] = df_tmp.apply(lambda x: f'{data_folder}/{city}/satellite/satellite/{x.sat_np2}', axis=1)
            df_tmp["path_sat_np3"] = df_tmp.apply(lambda x: f'{data_folder}/{city}/satellite/satellite/{x.sat_np3}', axis=1)
